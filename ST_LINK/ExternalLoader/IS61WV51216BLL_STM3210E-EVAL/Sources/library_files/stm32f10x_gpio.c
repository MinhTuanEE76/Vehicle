...
void GPIO_EventOutputCmd(FunctionalState NewState)
{
  ...
  *(__IO uint32 *) EVCR_EVOE_BB = (uint32)NewState;
}
...
void GPIO_EXTILineConfig(uint8_t GPIO_PortSource, uint8_t GPIO_PinSource)
{
  ...
  tmp = ((uint32)0x0F) << (0x04 * (GPIO_PinSource & (uint8_t)0x03));
  AFIO->EXTICR[GPIO_PinSource >> 0x02] &= ~tmp;
  AFIO->EXTICR[GPIO_PinSource >> 0x02] |= (((uint32)GPIO_PortSource) << (0x04 * (GPIO_PinSource & (uint8_t)0x03)));
}
...
{ 
  ...
  *(__IO uint32 *) MAPR_MII_RMII_SEL_BB = GPIO_ETH_MediaInterface; 
}
