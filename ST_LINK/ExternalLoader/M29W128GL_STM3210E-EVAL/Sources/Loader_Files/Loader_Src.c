...
/**
  ...
  */
int Read (uint32 Address, uint32 Size, uint16_t* Buffer)
{    
  uint32 InternalAddr = Address - StartAddresse;
  uint32 ReadedData = 0;
  uint32 Counter = 0;   
...
/**
  ...
  * Note: Mandatory for all types except SRAM and PSRAM	
  */
int Write (uint32 Address, uint32 Size, uint16_t* Buffer)
{  
  uint32 InternalAddr = Address - StartAddresse;
  uint32 WritedData = 0;
  uint32 Counter = 0; 
...
/**
  ...
  * Note: Not Mandatory for SRAM PSRAM and NOR_FLASH
  */
int SectorErase (uint32 EraseStartAddress ,uint32 EraseEndAddress)
{      
  uint32 BlockAddr;
...
/**
  ...
  */
uint32 CheckSum(uint32 StartAddress, uint32 Size, uint32 InitVal)
{
...
/**
  ...
  * Note: Optional for all types of device
  */
uint64_t Verify (uint32 MemoryAddr, uint32 RAMBufferAddr, uint32 Size, uint32 missalignement)
{
  uint32 InitVal = 0;
  uint32 InternalAddr = MemoryAddr - StartAddresse;
  uint32 VerifiedData = 0;
  uint16_t TmpBuffer = 0x0000;
  uint64_t checksum;
  Size*=4;
        
  checksum = CheckSum((uint32)MemoryAddr + (missalignement & 0xf), Size - ((missalignement >> 16) & 0xF), InitVal);
