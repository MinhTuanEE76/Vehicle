...
NOR_Status NOR_EraseBlock(uint32 BlockAddr);
NOR_Status NOR_EraseChip(void);
NOR_Status NOR_WriteHalfWord(uint32 WriteAddr, uint16_t Data);
NOR_Status NOR_WriteBuffer(uint16_t* pBuffer, uint32 WriteAddr, uint32 NumHalfwordToWrite);
NOR_Status NOR_ProgramBuffer(uint16_t* pBuffer, uint32 WriteAddr, uint32 NumHalfwordToWrite);
uint16_t NOR_ReadHalfWord(uint32 ReadAddr);
void NOR_ReadBuffer(uint16_t* pBuffer, uint32 ReadAddr, uint32 NumHalfwordToRead);
NOR_Status NOR_ReturnToReadMode(void);
NOR_Status NOR_Reset(void);
NOR_Status NOR_GetStatus(uint32 Timeout);
